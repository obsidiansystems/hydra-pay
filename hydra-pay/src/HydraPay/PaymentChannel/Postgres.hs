...
getPaymentChannelsInfo a addr = do
  ...
  results <- Db.runQueryInTransaction a $ \conn -> runBeamPostgres conn $ runSelectReturningList $ select $ do
    ...
    head_ <- leftJoin_ (all_ $ Db.db ^. Db.db_heads) (\head_ -> val_ True)
    -- head_ <- leftJoin_ (all_ $ Db.db ^. Db.db_heads) (\head_ -> (paymentChannel ^. Db.paymentChannel_head) `references_` head_)
...
dbPaymentChannelToInfo :: Api.AddressAny -> Maybe Db.HydraHead -> Db.PaymentChannel -> PaymentChannelInfo
dbPaymentChannelToInfo addr hh pc =
  ...
  where
    status =
      case hh >>= Db._hydraHead_secondBalance of
...
getPaymentChannelDetails a addr pcId = do
  ...
  Db.runBeam a $ runExceptT $ do
    ...
    case mHead of
      ...
      Just h -> do
        let
          isFirst = pc ^. Db.paymentChannel_first == Api.serialiseAddress addr
...
createPaymentChannel a (PaymentChannelConfig name first second amount chain) = do
  ...
  results <- runInsertReturningList $ insert (Db.db ^. Db.db_paymentChannels) $
  ...
  -- TODO(skylar): Fix this before you push.
  pure $ unSerial . Db._paymentChannel_id $ head results
  where
