...
JQ_CMD=
if [[ -n ${3} ]]; then
    echo >&2 "Using provided jq command: ${3}"
    $(${3} --version > /dev/null)
    JQ_CMD=${3}
fi
...
function seedFaucet() {
    ...
    FAUCET_TXIN=$(ccli query utxo --address ${FAUCET_ADDR} --out-file /dev/stdout | ${JQ_CMD} -r 'keys[0]')
    ...
    while [[ "$(ccli query utxo --tx-in "${SEED_TXIN}" --out-file /dev/stdout | ${JQ_CMD} ".\"${SEED_TXIN}\"")" = "null" ]]; do
        sleep 1
        echo -n >&2 "."
    done
    echo >&2 "Done"
