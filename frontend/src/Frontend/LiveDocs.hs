...
-- | 
module Frontend.LiveDocs where
...
data HydraPayConnection t = HydraPayConnection
  { connection_authenticated :: Dynamic t Bool
  , connection_nextId :: Dynamic t Int64
  , connection_subscriptions :: Event t ServerMsg
  , connection_responses :: Event t (Tagged ServerMsg)
  }

fanBool :: Reflex t => Event t Bool -> (Event t (), Event t ())
fanBool ev = (failure, success)
  where
    success = fmapMaybe (bool Nothing (Just ())) ev
    failure = fmapMaybe (bool Nothing (Just ()) . not) ev

auth :: (Reflex t, MonadHold t m, EventWriter t [ClientMsg] m) => HydraPayConnection t -> Event t T.Text -> m (Event t Bool)
auth conn authKey = do
  response <- requester (connection_nextId conn) (connection_responses conn) $ Authenticate <$> authKey
  pure $ updated (connection_authenticated conn)

runLiveDocs :: (SetRoute t (R FrontendRoute) m, TriggerEvent t m, MonadJSM m, EventWriter t [ClientMsg] m, MonadFix m, MonadHold t m, MonadJSM (Performable m), PerformEvent t m, PostBuild t m, DomBuilder t m) => HydraPayConnection t -> m ()
runLiveDocs conn@(HydraPayConnection authenticated lastTagId subscriptions responses) = do
  rec
    showDisc <- holdDyn True $ False <$ close
    let
      discStyles b =
        T.intercalate " " ["transition-all text-gray-700 absolute w-full h-full flex justify-center items-start z-10 backdrop-blur-md py-16"
                          , bool "opacity-0 pointer-events-none" "opacity-100 pointer-events-auto" b
                          ]
    close <- elDynClass "div" (discStyles <$> showDisc) $ do
      elClass "div" "p-8 bg-white drop-shadow-lg rounded-lg" $ do
        elClass "div" "text-xl font-semibold mb-4" $ text "Welcome to your Hydra Pay Instance"
        elClass "ul" "leading-loose list-inside list-disc mb-8" $ do
          elClass "li" "" $ text "You have been given access to a private Hydra Pay Instance running on a shared private Devnet"
          elClass "li" "" $ text "You have 4 personal addresses containing 1000 ADA, that will replenish everytime the devnet resets"
          elClass "li" "" $ text "The devnet can reset intermittently, if you experience strange behavior, a refresh is the best way to go (Handling restarts is on the way)"
          elClass "li" "" $ text "If you run out of funds or have an issue, wait for the devnet to reset"
          elClass "li" "" $ text "If you have persistent issues or questions, reach out to your key provisioner"

        checked <- elClass "label" "block w-full label cursor-pointer flex flex-row items-center hover:bg-green-100 text-green-700 font-semibold rounded-md" $ do
          text "I have read and understand the above"
          ie <- inputElement $ def
            & initialAttributes .~ ("class" =: "ml-4" <> "type" =: "checkbox")

          pure $ _inputElement_checked ie

        let
          mkClasses confirmed =
            T.intercalate " " [ "mt-2 flex-grow-0 rounded-md px-4 py-1 text-center text-white font-bold border-2 border-green-600"
                              , bool "cursor-not-allowed bg-gray-500 border-gray-600" "cursor-pointer bg-green-500 border-green-600" confirmed
                              ]

        (buttonEl, _) <- elDynClass' "button" (mkClasses <$> checked) $ text "Continue"

        let
          closeThis = gate (current checked) $ domEvent Click buttonEl

        pure closeThis

  key <- liftJSM $ do
    val <- jsg "localStorage" ^. js1 "getItem" "apiKey"
    fromJSValUnchecked val

  pb <- getPostBuild
  (authFailure, _) <- fmap fanBool $ auth conn $ key <$ pb
  setRoute $ FrontendRoute_Login :/ () <$ authFailure
  requester lastTagId responses $ Authenticate key <$ pb
  elClass "div" "w-full h-full flex flex-row text-gray-700" $ do
    activeHeads authenticated lastTagId subscriptions responses
    elClass "div" "flex-grow" $ monitorView lastTagId responses True key
...
activeHeads authenticated lastTagId subscriptions responses = do
  elClass "div" "flex-grow p-4" $ elClass "div" "max-w-lg" $ do
    {-elClass "div" "" $ mdo
      msg <- requester lastTagId responses $ GetRestartTime <$ leftmost [ () <$ gate (current authenticated) (updated authenticated)
                                                                        , devnetRestarted
                                                                        ]
      let
        restartTime = fmapMaybe (preview _RestartTime . tagged_payload) msg
        devnetRestarted = fmapMaybe (preview _DevnetRestarted) subscriptions

      timeUpdate <- performEvent $ liftIO getCurrentTime <$ leftmost [() <$ gate (current authenticated) (updated authenticated), devnetRestarted]
      now <- holdDyn Nothing $ Just <$> timeUpdate
      nextRestart <- holdDyn Nothing $ Just <$> restartTime

      tick <- tickLossyFromPostBuildTime 1
      elapsed <- foldDyn ($) (0 :: NominalDiffTime) $ leftmost [ (1+) <$ tick
                                                               , const 0 <$ devnetRestarted
                                                               ]

      let
        nowDyn = liftM2 (flip addUTCTime) <$> now <*> (Just <$> elapsed)
        currentDiff = liftM2 diffUTCTime <$> nextRestart <*> nowDyn

      elClass "div" "text-5xl " $ dynText $ ffor currentDiff $ \case
        Just s ->
          tShow minutes <> ":" <> (if (T.length secondsText == 1) then "0" <> secondsText else secondsText)
          where
            secondsText = tShow seconds

            minutes :: Int32
            minutes = (round s :: Int32) `div` 60

            seconds :: Int32
            seconds =  (round s) - minutes * 60
        Nothing -> "Fetching time"
      elClass "div" "text-xl " $ text "Time until devnet restart"-}
    header "Your Heads"

    dyn_ $ ffor authenticated $ \case
      False -> elClass "span" "text-gray-400" $ text "Authenticate to monitor heads"
      True -> do
        subscribed <- foldDyn ($)  (mempty :: Set T.Text) $ leftmost [const mempty <$ restartHappened, Set.insert . headStatus_name <$> startedSubscription]
    ...
    where
      ...
      restartHappened = fmapMaybe (preview _DevnetRestarted) subscriptions
...
authentication ::
  ...
  => Dynamic t Int64 -> Event t (Tagged ServerMsg) -> T.Text -> m ()
authentication lastTagId serverMsg key = do
  elClass "div" "px-4 pb-4" $ do
    ...
    authRequest <- elClass "div" "ml-4 mb-4 flex flex-row" $ do
      ...
      elClass "div" "flex flex-col" $ do
        ie <- inputElement $ def
          ...
          & inputElementConfig_initialValue .~ key
...
monitorView ::
  ...
  , PerformEvent t m
  , EventWriter t [ClientMsg] m
  ) => Dynamic t Int64 -> Event t (Tagged ServerMsg) -> Bool -> T.Text -> m ()
monitorView lastTagId serverMsg isManagedDevnet key = do
  elClass "div" "h-full text-gray-700 lg:max-w-6xl max-w-4xl overflow-y-scroll rounded flex flex-col flex-shrink-0" $ do
    elClass "div" "p-4" $ do
      elClass "div" "text-3xl font-bold flex flex-row items-center justify-between" $ do
        el "div" $ text "🐲 Hydra Pay Live Documentation Instance"
    ...
    authentication lastTagId serverMsg key
